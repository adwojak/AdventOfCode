...
def part_one():
    return len([line for line in input_data if line.min_value <= line.password.count(line.letter) <= line.max_value])


def part_two():
    return len([line for line in input_data if bool(line.password[line.min_value] == line.letter) ^ bool(
        line.password[line.max_value] == line.letter)])
